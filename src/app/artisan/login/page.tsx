...
export default function ArtisanLogin() {
  ...
  const { setArtisan, triggerToast } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        triggerToast("Artisan studio session loaded successfully", "success");
        router.push("/artisan/dashboard");
        router.refresh();
      } else if (res.status === 403 && data.artisanId) {
        
        triggerToast("Email is unverified. Directing to verification screen...", "warning");
        router.push(`/artisan/verify?artisanId=${data.artisanId}`);
      } else {
        triggerToast(data.message || "Invalid credentials", "error");
      }
    } catch (error) {
    ...
    <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gradient-to-br from-emerald-50 via-[#FBFBFA] to-amber-50 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-1.5 text-xs text-slate-500 text-left">
          Log in to manage creations, configure pricing, track shipments and view sales statistics.
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white/80 backdrop-blur-md py-8 px-4 border border-slate-200/50 shadow-xl rounded-2xl sm:px-10">
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Link href="/artisan/forgot-password" className="text-xs font-medium text-brand-green hover:underline">
                  Forgot Password?
                </Link>
              </div>
          ...
          <div className="mt-6 border-t border-slate-100 pt-5 text-center text-xs font-medium text-slate-500">
            Want to sell bespoke creations?{" "}
            <Link href="/artisan/register" className="font-semibold text-brand-green hover:underline">
              Register as Artisan
            </Link>
          </div>
        </div>
